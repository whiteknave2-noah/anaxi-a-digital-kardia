...
"""
The Anaxi Protocol – Pure-SQLite variant (v2)
=============================================
Same architecture and reliability guarantees as the FAISS version,
but with zero FAISS dependency.

Retrieval uses a linear cosine scan over embeddings stored as BLOBs.
Adequate for a few thousand nodes per user.

Improvements mirrored from the FAISS version
--------------------------------------------
- PRAGMA foreign_keys = ON
- Deletions become proposals (not immediate hard deletes)
- Real axiom validators (destination + trajectory)
- Kardia history for trajectory assessments
- Proposal resolution API
...
class ConstitutionalMind:
    """
    Pure-SQLite multi-user cognitive mind.
    Public API matches the FAISS version so callers can swap implementations.
    """
    ...
    # Chain-of-custody tag for the observation log below -- bump this if the
    # prototype phrases, embedding model, or threshold ever change, so past
    # observations stay traceable to the classifier that produced them.
    AXIOM_CLASSIFIER_VERSION = "1.0"
    # Cadence for the constitutional-flag report (a plain comparison, not a
    ...
    def _init_core_tables(self) -> None:
        ...
        # Continuous observation log: the raw per-axiom similarity scores and
        # trajectory delta ARE computed already, every time an evolution is
        # attempted or a proposal is resolved -- this just keeps them instead
        # of discarding everything but the pass/fail outcome. Pure record,
        # no judgment about which axiom "caused" anything.
        self.cursor.execute(
            """
        ...
        # Per-turn record of the generation parameters actually in effect.
        # Execution record, not an explanation -- no prompt content stored,
        # only what was used to generate the response.
        self.cursor.execute(
            """
    ...
    def _store_embedding(self, user_id: str, node_id: str, text: str) -> None:
    ...
    def _propose_delete(
        ...
        """Create a pending delete proposal only if safe and non-duplicate."""
        self.cursor.execute(
    ...
    def retrieve_waking_context(
        ...
        top_k: int = 12,
        min_similarity: float = 0.35,
        asserted_before: Optional[int] = None,
    ) -> str:
        """`asserted_before` (default None = unrestricted, the historical
        behavior) limits both seed and neighbor nodes to those whose own
        asserted_at is non-NULL and strictly earlier than it.  It is the
        read-time seam for the legacy owner-private Family compatibility
        rule: nodes of unknown or later assertion time are never returned."""
        query_vec = self._embed_text(user_prompt)
        seed_hits = self._linear_search(user_id, query_vec, top_k, min_similarity)
        if not seed_hits:
            return ""

        seed_ids = [nid for nid, _ in seed_hits]
        placeholders = ",".join("?" * len(seed_ids))
        seed_time_clause = "" if asserted_before is None else "AND asserted_at IS NOT NULL AND asserted_at < ? "
        neighbor_time_clause = "" if asserted_before is None else "AND n.asserted_at IS NOT NULL AND n.asserted_at < ? "
        time_params = [] if asserted_before is None else [asserted_before]
        self.cursor.execute(
            f"""
            SELECT id, label, type, description, salience_score
            FROM nodes
            WHERE user_id = ? AND id IN ({placeholders}) {seed_time_clause}
            ORDER BY salience_score DESC
            """,
            [user_id] + seed_ids + time_params,
        )
        ...
        for node_id, label, node_type, description, score in seed_nodes:
            ...
            self.cursor.execute(
                ...
                WHERE e.user_id = ? AND (e.source = ? OR e.target = ?) AND n.id != ?
                """ + neighbor_time_clause,
                [user_id, node_id, node_id, node_id] + time_params,
            )
    ...
    def _set_sleep_watermark(self, user_id: str, ts: int) -> int:
        ...
        returning the new count so the caller can decide whether this cycle
        is a multiple of CONSTITUTIONAL_FLAG_INTERVAL."""
        self.cursor.execute(
            """
    ...
    def execute_sleep_consolidation(
        ...
        try:
            ...
            # Deletions → proposals (protected types refused at proposal time)
            ...
            if "upsert_nodes" in data:
                for incoming in data["upsert_nodes"]:
                    ...
                    if exact:
                        ...
                        if len(matches) == 1:
                            existing_id, existing_asserted_at = matches[0]
                    ...
                    self.cursor.execute(
                        ...
                        """,
                        (
                            user_id, node_id, label, incoming["type"],
                            incoming["salience_score"], description,
                            current_time, raw_log_timestamp,
                        ),
                    )
                    self._store_embedding(user_id, node_id, description)
            ...
            self.cursor.execute(
                f"""
                UPDATE nodes SET salience_score = salience_score * 0.9
                WHERE user_id = ? AND last_accessed_timestamp < ?
                  AND type NOT IN ({placeholders})
                """,
                [user_id, previous_sleep_ts] + list(self.CORE_PROTECTION_TYPES),
            )

            # Decayed nodes → proposals
            self.cursor.execute(
                f"""
                SELECT id FROM nodes
                WHERE user_id = ? AND salience_score < ?
                  AND type NOT IN ({placeholders})
                """,
                [user_id, self.SALIENCE_DELETE_THRESHOLD] + list(self.CORE_PROTECTION_TYPES),
            )
            for (decayed_id,) in self.cursor.fetchall():
                if self._is_suppressed(user_id, decayed_id, "delete_node"):
                    continue
                self._propose_delete(
                    user_id=user_id,
                    node_id=decayed_id,
            ...
            self.conn.rollback()
            self._log_cognitive_failure(user_id, "MALFORMED_REPRESENTATION", rem_llm_json_output, str(e))
            return {"status": "failed", "reason": "Structural interpretation error safely caught."}
        except sqlite3.Error as e:
            self.conn.rollback()
            self._log_cognitive_failure(user_id, "DATABASE_TRANSACTION_CRASH", rem_llm_json_output, str(e))
    ...
    def _run_graph_garbage_collector(self, user_id: str) -> None:
        self.cursor.execute(
            ...
            DELETE FROM edges
            WHERE user_id = ?
              AND (source NOT IN (SELECT id FROM nodes WHERE user_id = ?)
                OR target NOT IN (SELECT id FROM nodes WHERE user_id = ?))
            """,
            (user_id, user_id, user_id),
        )
        self.cursor.execute(
            "DELETE FROM edges WHERE user_id = ? AND source = target", (user_id,)
        )
    ...
    def get_current_kardia(self, user_id: str) -> Dict[str, str]:
        ...
        if not row:
            ...
            self.cursor.execute(
                ...
                VALUES (?, ?, ?, ?, ?, ?)
                """,
                (user_id, default["moral_valve"], default["volitional_channel"],
                 default["affective_stance"], default["aesthetic_valve"], current_time),
            )
            self.conn.commit()
            return default
        return {
    ...
    def _validate_axioms_destination(self, new_kardia: Dict[str, str]) -> Optional[str]:
        """Hybrid: phrase filter then semantic prototype similarity."""
        ...
        deception_patterns = (
            "intentionally deceive", "intentional deception", "lie to the user",
            "deceive the user", "manipulate the user", "hide the truth",
            "conceal the truth", "mislead deliberately", "never seek clarity",
        )
        if any(p in combined for p in deception_patterns):
            return "Destination violates Axiom 3 (Radical transparency) [phrase]."

        fragmentation_patterns = (
            "multiple personalities", "fragment into", "split identity",
            "conflicting selves", "incoherent personas",
        )
        if any(p in combined for p in fragmentation_patterns):
            return "Destination violates Axiom 1 (Coherence over chaos) [phrase]."

        lock_patterns = (
            "permanently lock", "permanent lock", "no further change",
            "freeze identity", "identity is final", "cannot be revised",
    ...
    def _kardia_delta(
        ...
        """Semantic change magnitude via cosine distance on Kardia embeddings."""
    ...
    def _log_axiom_observation(
        ...
        """
        Continuous observation logging. Computes and stores the raw
        similarity of new_kardia to all three axioms' violation prototypes
        -- not just whichever one the gate happened to assess before
        returning -- plus the trajectory delta if an old_kardia is given.

        This is deliberately a separate, unconditional measurement, decoupled
        from _validate_axioms_destination's early-return gating logic: it
        does not judge or threshold anything, it just keeps the numbers that
        were already computed. Uses self.cursor, so it rides along with
    ...
    def _maybe_generate_constitutional_observation(self, user_id: str, cycle_count: int) -> None:
        """
        Every CONSTITUTIONAL_FLAG_INTERVAL cycles, compare the most recent
        window of axiom_observations against the window before it and leave
        a plain factual comparison as a pending 'constitutional_observation'
        proposal. "Dominant axiom" per event = argmax of the three stored
        similarity scores -- the same similarity machinery already trusted
        elsewhere, not a fresh classifier.

        Deliberately makes no materiality judgment (no "this shifted
        significantly" threshold) -- same reason calibration thresholds are
        deferred: no observed data yet exists to justify picking a cutoff.
        The numbers are reported; whether they're material is for the human
        reviewing proposals to decide.
        """
        ...
        if len(rows) < window:
            return  # not enough observation history yet to say anything
    ...
    def log_turn_generation_controls(
        ...
        Execution record, not an explanation of why the response came out
        the way it did -- and deliberately stores no prompt content, only
        the parameters, to avoid quietly taking on raw-content retention
        this project has already flagged as a real cost elsewhere."""
        try:
            self.cursor.execute(
                """
    ...
    def log_turn_generation_controls_for_event(
        self, user_id: str, controls: Dict[str, Any], *,
        model_revision_id: str, pipeline_id: str, event_id: str, timestamp: int,
    ) -> None:
        """Native-path sibling of log_turn_generation_controls().
        Called ONLY after a native turn's canonical provenance
        transaction has already committed -- never before -- with the
        real event_id/pipeline_id/model_revision_id already known.
        Unlike the historical method above, this one does NOT swallow
        exceptions: a failure here is exactly what reconcile_native_turn()
        (native_provenance_writer.py) exists to detect and repair, so it
        must be visible to the caller, not hidden."""
        self.cursor.execute(
            """
            INSERT INTO turn_generation_log
                (user_id, timestamp, temperature, top_p, style_instruction,
                 model_revision_id, pipeline_id, epoch_id, event_id)
            VALUES (?, ?, ?, ?, ?, ?, ?, NULL, ?)
            """,
            (
                user_id, timestamp,
                controls.get("temperature"), controls.get("top_p"),
                controls.get("style_instruction"),
                model_revision_id, pipeline_id, event_id,
            ),
        )
        self.conn.commit()
    ...
    def _validate_axioms_trajectory(
        ...
        allowed = max(0.12, (1.0 - inertia) * 1.2)
    ...
    def govern_identity_revision(
        ...
        if traj_err:
                    ...
                    json.dumps(
                        {
                            ...
                            "choice": choice,
                            # None is deliberate and honest for legacy/direct
                            # callers that cannot establish an originating
                            # observation; never substitute the current time.
                            "triggering_observation_timestamp": triggering_observation_timestamp,
                        }
                    ),
                    traj_err if choice == "ADJUST" else f"REWRITE with large delta: {traj_err}",
                    int(time.time()),
                ),
            )
            self._log_axiom_observation(
            ...
            self.conn.commit()
            return f"{'REWRITE' if choice == 'REWRITE' else 'Evolution'} deferred for review: {traj_err}"
        ...
        self.cursor.execute(
            ...
            VALUES (?, ?, ?, ?, ?, ?)
            """,
            (user_id, old_kardia["moral_valve"], old_kardia["volitional_channel"],
             old_kardia["affective_stance"], old_kardia["aesthetic_valve"], current_time),
        )
        self.cursor.execute(
            """
            INSERT INTO active_kardia
                (user_id, moral_valve, volitional_channel, affective_stance, aesthetic_valve, updated_at)
            VALUES (?, ?, ?, ?, ?, ?)
            ON CONFLICT(user_id) DO UPDATE SET
            ...
            """,
            (user_id, new_kardia["moral_valve"], new_kardia["volitional_channel"],
             new_kardia["affective_stance"], new_kardia["aesthetic_valve"], current_time),
        )
        self._log_axiom_observation(
            user_id, new_kardia, event_type="evolve_attempt",
    ...
    def list_pending_proposals(self, user_id: str) -> List[Dict[str, Any]]:
        self.cursor.execute(
            ...
            FROM proposals WHERE user_id = ? AND status = 'pending'
    ...
    def _is_suppressed(self, user_id: str, node_id: str, proposal_type: str) -> bool:
        """A node is suppressed from a given proposal type if the most
        recent resolved proposal of that exact type, for that exact
        node, was a rejection with no subsequent reconsideration.
        Derives suppression entirely from the existing proposals
        table -- no schema change. Per design note section 4: node +
        proposal_type + prior explicit rejection, deliberately
        narrower than suppressing the node itself so other proposal
        types remain unaffected. id DESC as a secondary sort key
        makes this deterministic if two events share a resolved_at
        timestamp."""
        rows = self.cursor.execute(
            """
            SELECT payload, status FROM proposals
            WHERE user_id = ? AND proposal_type = ? AND status IN ('rejected', 'reconsidered')
            ORDER BY resolved_at DESC, id DESC
            """,
            (user_id, proposal_type),
        ).fetchall()
        for payload_raw, status in rows:
            if json.loads(payload_raw).get("node_id") == node_id:
                return status == "rejected"
        return False

    def reconsider_proposal(self, user_id: str, node_id: str, proposal_type: str, current_time: int) -> Dict[str, str]:
        """Deliberate human action only -- never inferred by the
        scheduler from silence, decay, or elapsed time. Records a new
        governance event; never mutates the original rejection. Does
        not itself accept or reject anything -- only restores
        eligibility for the ordinary proposal machinery to run again
        on its next cycle."""
        if not self._is_suppressed(user_id, node_id, proposal_type):
            return {"status": "failed", "reason": f"No active suppression found for {node_id}/{proposal_type}."}
        payload = json.dumps({"node_id": node_id})
        self.cursor.execute(
            """
            INSERT INTO proposals (user_id, proposal_type, payload, reason, status, created_at, resolved_at)
            VALUES (?, ?, ?, ?, 'reconsidered', ?, ?)
            """,
            (user_id, proposal_type, payload, "Manual reconsideration -- suppression lifted.", current_time, current_time),
        )
        self.conn.commit()
        return {"status": "success", "message": f"Suppression lifted for {node_id}/{proposal_type}. Eligible again on next cycle."}

    def resolve_proposal(self, user_id: str, proposal_id: int, accept: bool) -> Dict[str, str]:
        self.cursor.execute(
            "SELECT proposal_type, payload, status FROM proposals WHERE id = ? AND user_id = ?",
            (proposal_id, user_id),
        )
        row = self.cursor.fetchone()
        if not row:
        ...
        if ptype == "delete_node":
            ...
            self.cursor.execute(
                "DELETE FROM nodes WHERE user_id = ? AND id = ?", (user_id, node_id)
            )
            ...
            # Reassess trajectory against whatever Kardia has become SINCE this
            # proposal was queued -- not just against the snapshot it was
            # compared to at creation time. Otherwise an old proposal can land
            # a bigger jump than the inertia gate would allow if evaluated
            # fresh right now.
            traj_err = self._validate_axioms_trajectory(old_kardia, new_kardia)
            if traj_err:
                self.cursor.execute(
